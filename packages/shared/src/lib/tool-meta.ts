export interface ToolMetaLabels {
...
const labelRegistry: Record<string, ToolMetaLabels> = {
  Bash: {
...
export function getToolLabels(toolName: string): ToolMetaLabels {
  const canonicalName = canonicalizeToolName(toolName)
  return (
    labelRegistry[canonicalName] ?? {
