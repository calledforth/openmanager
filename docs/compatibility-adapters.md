...
| `openSession`                      | Session summary + thread identities through `applySessionOpen`; transcript through `applySessionHistory` from `messages.listMetadata` + `messages.getContent` + pending interaction rows                                                   |
