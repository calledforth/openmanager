...
| **Active thread** (`active-thread-provider`) | The persisted session record on screen, message metadata with optimistic user messages (echoed by the environment client's per-thread outbox, keyed by the command id a send carries), the `StreamingMessagesStore`, and every turn command: `sendMessage`, `retrySend`, `abortSession`, `resolvePermission`, `resolveQuestion`, `resolvePlan`, `buildPlan`. | Which session is active (read from session state). |
