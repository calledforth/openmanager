...
| `workspaces`                     | Environment                                                                                                             | Registrations persist across restarts (migration 6 adds `last_used_at`). The canonical path, last use and an existence check are listed to clients; clients still address a workspace by ID (D9). |
...
| `audit_events`                   | Environment                                                                                                             | Security events (refusals, capability denials, token issue/revoke). Client id, command and outcome are queryable; secrets are never stored.               |
...
| none                                                | `environment_metadata`, `threads`, `turns`, `drafts`, `stash_items`, `authorized_clients`, `audit_events`, `event_streams`, `event_log` |
