...
The browser projects this single environment-client state into the shared chat stores. It does not listen to `stream:token` or `acp:event`, query Convex, or decide between local IPC and remote history based on ownership. The shared chat's legacy `activeThreadDriven` adapter field remains until the separate overlay retirement; both message stores here read the same projection.
